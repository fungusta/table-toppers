// CafeView.jsx — "The Cafe": cross-game overview.
// No hero header — content-first. Champions strip, league table across all
// three games, recent matches feed, at-a-glance stats.

const { useMemo: useMemoCafe } = React;

function CafeView({ matches, range, onPickPlayer, onRecord, onOpenHistory }) {
  const allStandings = useMemoCafe(
    () => computeStandings(PLAYERS, matches, 'cafe', range),
    [matches, range]
  );
  const recent = useMemoCafe(
    () => filterMatches(matches, 'cafe', range).slice(0, 6),
    [matches, range]
  );
  const totalGames = useMemoCafe(
    () => filterMatches(matches, 'cafe', range).length,
    [matches, range]
  );
  const champs = useMemoCafe(() => {
    return ['catan', 'carcassonne', 'monopoly'].map(g => {
      const s = computeStandings(PLAYERS, matches, g, range);
      return { game: g, top: s[0] };
    });
  }, [matches, range]);
  const longest = useMemoCafe(() => {
    return allStandings.reduce((m, s) => s.streak > (m?.streak ?? 0) ? s : m, null);
  }, [allStandings]);
  const mostActive = useMemoCafe(() => {
    return [...allStandings].sort((a,b) => b.played - a.played)[0];
  }, [allStandings]);
    ...
    <div className="caf-root">
      ...
      <section>
        ...
        <div className="caf-champs">
          {champs.map(c => (
            <CafeChamp key={c.game} game={c.game} top={c.top}
            ...
            <tbody>
              {allStandings.map((s, i) => (
                <tr key={s.player.id} onClick={() => onPickPlayer(s.player.id)}>
                  <td><span className={"caf-rank" + (i === 0 ? ' caf-rank-1' : '')}>{i + 1}</span></td>
                  <td>
                  ...
                  <td className="num"><strong style={{ color: '#9c4b34', fontSize: 15 }}>{s.wins}</strong></td>
          ...
          {recent.map(m => {
            ...
            return (
              <li key={m.id} className="caf-recent-row" onClick={() => onPickPlayer(w.id)}>
                ...
                <div>
                  <div className="caf-recent-title"><strong>{w.name}</strong> won {GAMES[m.game].label}</div>
...
function CafeChamp({ game, top, onClick }) {
  const glyph = <CafeGameGlyph id={game}/>;
  return (
    <button className="caf-champ" onClick={onClick} disabled={!top}>
      <div className="caf-champ-glyph">{glyph}</div>
      <div className="caf-champ-body">
        ...
        <div className="caf-champ-wins">
          {top ? `${top.wins} win${top.wins === 1 ? '' : 's'} · ${Math.round(top.winRate*100)}% rate` : 'no matches yet'}
        </div>
      </div>
      {top && (
