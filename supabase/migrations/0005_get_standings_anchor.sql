-- =========================================================
-- 0005_get_standings_anchor.sql
--
-- Replaces the hardcoded `v_today := date '2026-05-23'` anchor in
-- `get_standings` with a data-driven anchor: the most recent match in the
-- target group, falling back to `current_date` when the group has no
-- matches yet.
--
-- Rationale (per docs/superpowers/NEXT.md item 2, option C): mirrors the
-- TS-side `rangeCutoff(matches, range)` helper in web/src/data/data.ts so
-- SQL and client computations agree, and prevents the seed data's range
-- filters from going stale once the calendar advances past mid-2026.
-- =========================================================
