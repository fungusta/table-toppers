...
interface CarcassonneViewProps {
  matches: Match[];
  range: Range;
...
export function CarcassonneView({
  matches,
  range,
  onPickPlayer,
  onRecord,
  onOpenHistory,
}: CarcassonneViewProps) {
      ...
      <div className="carc-page">
        <header className="carc-illum-header">
          ...
          <h1 className="carc-illum-title">
            ...
            <span className="carc-illum-title-text">
              Chronicle of <em>Carcassonne</em>
            </span>
          </h1>
                ...
                {standings.map((s, i) => (
                  <tr key={s.player.id} onClick={() => onPickPlayer(s.player.id)}>
                    <td className="carc-td-rank">
                      {i === 0 ? (
                        <span className="carc-roman-1">I</span>
                      ) : (
                    ...
                    <td>
                      <div className="carc-td-name">
                        ...
                        <div>
                          ...
                          <div className="carc-td-name-handle">
                            — {s.player.handle.replace(/_/g, " ")}
                          </div>
                        </div>
                      </div>
                    </td>
        ...
        <footer className="carc-footer">
          <span>
            Champion of the realm: <strong>{top?.player.name || "—"}</strong>
          </span>
...
function CarcScoreboard({
  ...
  const byScore: Record<number, Standing[]> = {};
  standings.forEach(s => {
  ...
  return (
    <section className="carc-scoreboard-section">
      ...
      <p className="carc-scoreboard-cap">
        Each meeple sits on the number of games its player has won.
      </p>
      <div className="carc-scoreboard-wrap">
        <svg
          viewBox={`0 0 ${W} ${H}`}
          className="carc-scoreboard-svg"
        ...
        >
          ...
          <rect
            ...
            strokeWidth="3"
          />
          <rect x="2" y="2" width={W - 4} height={H - 4} rx="6" fill="url(#carc-sb-grain)" />
              ...
              <g key={`tile-${n}`}>
                ...
                {milestone && (
                  <g transform={`translate(${x + TW - 22} ${y + TH - 22})`} opacity=".55">
                    <path
                      d="M0 14 L0 6 L3 6 L3 2 L6 2 L6 6 L9 6 L9 2 L12 2 L12 6 L15 6 L15 14 Z"
                      fill="#5c4329"
                    />
                  </g>
                )}
          ...
          {Object.entries(byScore).flatMap(([scoreStr, players]) => {
...
function CarcMeepleSvg({ color, size = 24 }: { color: string; size?: number }) {
  return (
...
function CarcTile({
  ...
  return (
    <button
      className={"carc-tile" + (rank === 1 ? " carc-tile-top" : "")}
      onClick={onClick}
    >
      <div className="carc-tile-art">
        <CarcTileArt seed={s.player.id} />
      ...
      <div className="carc-tile-foot">
        ...
        <div className="carc-tile-wins">
          {s.wins} <span>wins</span>
        </div>
      </div>
    </button>
  );
...
function CarcTileArt({ seed }: { seed: string }) {
  const tiles = [
...
function CarcMeeple({ color, size = 24 }: { color: string; size?: number }) {
  return (
...
function CarcOrnamentCorner({ flip }: { flip?: boolean }) {
  return (
    <svg
      viewBox="0 0 60 60"
      width="60"
...
function CarcDivider({ flip }: { flip?: boolean }) {
  return (
    <svg
      viewBox="0 0 120 12"
      className="carc-divider-svg"
      style={{ transform: flip ? "scaleX(-1)" : "none" }}
    >
