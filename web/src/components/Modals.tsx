...
export function PlayerProfileModal({ playerId, matches, onClose, theme, onPickPlayer }: PlayerProfileModalProps) {
  const open = !!playerId;
  const player: Player | null = open ? playerById(playerId!) : null;
...
export function RecordMatchModal({ open, onClose, theme, onSubmit, defaultGame }: RecordMatchModalProps) {
      ...
      {step === 1 && winner ? (
        <div className="rm-success">
          ...
          <div className="rm-success-sub">
            {playerById(winner).name} beat {selectedPlayers.length - 1} other
